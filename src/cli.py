...
def from_file(filepath, output):
    ...
    with open(output, 'w') as output_file:
        ...
        for user_id, customer_name in find_customers_in_radius(filepath, yield_json_from_file):
...
def from_url(url, output):
    ...
    with open(output, 'w') as output_file:
        output_file.write("user_id, name\n")
        for user_id, customer_name in find_customers_in_radius(url, yield_json_from_url):
            output_file.write(f"{user_id}, {customer_name}\n")
    click.echo(f"Customer Invite List Outputted at [{output}]")
